...
// icon-guess-the-word's room.js. Simpler than a multi-role implementation
// because only the Host ever acts — the Display is a pure render target and
// never sends a game action, so there's no per-connection role/permission
// system here at all (see plan.md's Networking model).

const ID_PREFIX = 'eggspress-room-'; // distinct from every sibling game's own prefix
...
//                             field) plus the Host's clock at broadcast time,
//                             so the Display can offset-correct its own
//                             timer countdown to match the Host's.
